...
import { muscleLabel, MUSCULOS_SUGERIDOS } from "@/lib/muscleLabels";
import { equipmentLabel } from "@/lib/equipmentLabels";
import { useExerciseFilterOptions } from "@/lib/useExerciseSearch";
 ...
 *
 * Vivía dentro de la pantalla de ejercicios del entrenador. Se saca aquí porque también se
 * abre desde la importación de rutinas: cuando un ejercicio leído de una captura no está
 * en la biblioteca, se crea en el momento con el nombre ya puesto, sin salir del flujo.
 *
 * Solo para entrenadores: las dos pantallas que lo abren cuelgan de /coach, cuyo layout
 * echa a quien no tenga rol de entrenador.
 */
export default function ExerciseForm({ existingExercises, exercise, initialName, onClose, onSaved }: {
  existingExercises: any[];
  exercise?: any;
  /** Nombre con el que abrir el formulario, para crear sobre lo que se leyó en una captura. */
  initialName?: string;
  onClose: () => void;
  /** Recibe el ejercicio guardado, para que quien lo abrió pueda usarlo en el acto. */
  onSaved: (guardado?: any) => void;
}) {
  const palette = usePalette();
  const { muscles: musculosEnUso, equipment: equipmentOptions } = useExerciseFilterOptions();
  // Las categorías salen de los ejercicios que ya existen, así que una nueva no aparecería
  // hasta que alguien la escribiera a mano. Las sugeridas se ofrecen siempre.
  const muscles = Array.from(new Set([...musculosEnUso, ...MUSCULOS_SUGERIDOS])).sort();
  ...
  async function handleSubmit(e: React.FormEvent, skipImage = false) {
    ...
    setSaving(true);
    setError("");
    // Aquí se pedía el usuario para nada: `uid` se calculaba y no se usaba en ninguna
    // parte, y el `!` hacía reventar el guardado si la sesión no estaba lista. Quien
    // comprueba el permiso es la ruta del servidor.
  ...
  // 320 va por encima de las ventanas modales (200) y del buscador a pantalla completa
  // (300). Desde la importación este formulario se abre ENCIMA de la ventana de revisión, y
  // con el z-index que tenía antes quedaba detrás: no se podía ni tocar el botón de guardar.
  return (
    <Overlay onClose={onClose} zIndex={320}>
