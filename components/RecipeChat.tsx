...
export default function RecipeChat() {
  ...
  /**
   * Empezar de cero borra TODO el historial de la conversación, y no se puede deshacer.
   * Antes lo hacía sin preguntar, desde un botón pequeño en la esquina — el resto de la
   * app confirma cualquier borrado.
   */
  async function newConversation() {
    ...
    const { error: deleteError } = await supabase.from("chat_messages").delete().eq("user_id", auth.user.id);
    if (deleteError) { setError("No pudimos borrar la conversación anterior, intenta de nuevo."); return; }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 48px)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        ...
        <button onClick={() => setConfirmNew(true)} title="Nueva conversación" style={{
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <button onClick={sendMessage} disabled={sending || (!input.trim() && !stagedImage)} style={{
          ...
          {sending ? <Loader2 size={16} className="ft-spin" /> : <Send size={16} />}
        </button>
      </div>

      {confirmNew && (
        <Overlay onClose={() => setConfirmNew(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ ...palette.modalPanel, padding: 22, width: "100%", maxWidth: 340 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <Button variant="danger" fullWidth onClick={newConversation}>Sí, empezar de cero</Button>
            </div>
          </div>
        </Overlay>
      )}
